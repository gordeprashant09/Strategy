...
# ══════════════════════════════════════════════════════════════════════════════
INSTRUMENT_MAP = {
    # ── NSE Indices (traded as FUT on NFO) ──────────────────────────────────
    ...
    # ── BSE Indices (traded as FUT on BFO) ──────────────────────────────────
...
def get_instrument_token(kite, symbol):
    ...
    if inst_type == "FUT":
        ...
        if active.empty:
            # Fallback: also try name/underlying column for index futures
            # (NFO/BFO sometimes lists them under a different tradingsymbol format)
            ...
            if active.empty:
                avail = df[df["instrument_type"]=="FUT"]["tradingsymbol"].str[:len(prefix)+2].unique()[:10].tolist()
                raise RuntimeError(
                    f"No {sym} futures found on {exchange}. "
                    f"Sample FUT symbols on {exchange}: {avail}. "
        ...
        candidates = df[mask]
        if candidates.empty:
            # Give a helpful hint if this looks like an index
            index_hints = {"NIFTY","BANKNIFTY","FINNIFTY","SENSEX","BANKEX",
                           "MIDCPNIFTY","NIFTYNXT50","SENSEX50"}
            if sym in index_hints:
                raise RuntimeError(
                    f"'{sym}' is an index — it cannot be looked up as EQ. "
                    f"It should be in INSTRUMENT_MAP with inst_type='FUT'. "
                    f"Please add it or check the INSTRUMENT_MAP at the top of this file."
                )
            raise RuntimeError(
                f"No EQ instrument found for '{sym}' on {exchange}. "
                f"Check that the symbol is spelled correctly and listed on {exchange}."
            )
