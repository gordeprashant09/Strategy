...
<style>
    ...
    .index-card {
        background: linear-gradient(145deg, #111827, #1c2232);
    ...
    .index-card:hover { border-color: #3d5a80; }
    .index-name { font-size: 13px; color: #8899aa; font-weight: 600; letter-spacing: 1px; text-transform: uppercase; }
    .index-ltp  { font-size: 28px; font-weight: 800; margin: 2px 0; }
    .index-chg-pos { color: #00e676; font-size: 15px; font-weight: 700; }
    .index-chg-neg { color: #ff5252; font-size: 15px; font-weight: 700; }
    .index-chg-neu { color: #aaa;    font-size: 15px; font-weight: 700; }
    ...
    .param-label { font-size: 11px; color: #607080; text-transform: uppercase; letter-spacing: 0.8px; }
    .param-value { font-size: 17px; font-weight: 700; color: #e0e8f0; }
    .param-sub   { font-size: 11px; color: #556677; margin-top: 2px; }

    .badge-bull { background: #0a2e16; color: #00e676; border: 1px solid #00e676;
                  border-radius: 5px; padding: 2px 8px; font-size: 11px; font-weight: 800; }
    .badge-bear { background: #2e0a0a; color: #ff5252; border: 1px solid #ff5252;
                  border-radius: 5px; padding: 2px 8px; font-size: 11px; font-weight: 800; }
    .badge-neut { background: #1a1e2a; color: #aabbcc; border: 1px solid #334455;
                  border-radius: 5px; padding: 2px 8px; font-size: 11px; font-weight: 800; }
    .badge-warn { background: #2e2200; color: #ffcc02; border: 1px solid #ffcc02;
                  border-radius: 5px; padding: 2px 8px; font-size: 11px; font-weight: 800; }

    .section-header {
        font-size: 17px; font-weight: 800; color: #69f0ae;
        border-left: 3px solid #00e676; padding-left: 10px; margin: 12px 0 8px 0;
    }

    .range-bar-outer {
        background: #1a2030; border-radius: 6px; height: 8px; margin: 6px 0; overflow: hidden;
    }
    .range-bar-inner {
    ...
    .vix-box {
        background: linear-gradient(135deg, #1a1020, #200d2e);
        border: 1px solid #4a1080;
        border-radius: 12px; padding: 14px 18px; text-align: center;
    }
    .vix-val { font-size: 32px; font-weight: 900; color: #ce93d8; }

    .trend-row { display: flex; align-items: center; gap: 8px; margin: 4px 0; }
    .trend-dot-bull { width:10px; height:10px; border-radius:50%; background:#00e676; display:inline-block; }
    .trend-dot-bear { width:10px; height:10px; border-radius:50%; background:#ff5252; display:inline-block; }
    .trend-dot-neut { width:10px; height:10px; border-radius:50%; background:#aaa; display:inline-block; }

    div[data-testid="stDataFrame"] { border-radius: 10px; }
    .stButton>button {
        background: linear-gradient(135deg, #0077cc, #003380);
        color: white; border: none; border-radius: 8px;
        font-weight: bold; padding: 8px 24px;
    }
    .stButton>button:hover { background: linear-gradient(135deg, #0099ff, #0055aa); }
...
#  HEADER
# ═══════════════════════════════════════════════════════════════════════════════
st.markdown("# 📊 Index Quant Dashboard")
st.markdown("**MFT Trading — NIFTY · BANKNIFTY · FINNIFTY · MIDCPNIFTY · SENSEX**")
...
h1.metric("🕐 IST Time",   now_ist.strftime("%H:%M:%S"))
h2.metric("📈 Market",     "🟢 OPEN" if market_open else ("🟡 PRE-OPEN" if pre_open else "🔴 CLOSED"))
h3.metric("📅 Date",       now_ist.strftime("%d %b %Y"))
h4.metric("👤 Account",    USER_ID)
st.divider()
...
if fetch_btn or "idx_spot" in st.session_state:
    ...
    for col, (name, cfg) in zip(cols, INDEX_CONFIG.items()):
        ...
        chg_cls  = "index-chg-pos" if chg_p > 0 else ("index-chg-neg" if chg_p < 0 else "index-chg-neu")
        chg_sign = "+" if chg_p >= 0 else ""
        gap_sign = "+" if gap_p >= 0 else ""

        # Range bar
        rp_clipped = max(0, min(100, rp))
        bar_color  = "#00e676" if chg_p >= 0 else "#ff5252"

        with col:
            st.markdown(f"""
            <div class="index-card">
                <div class="index-name">{cfg['icon']} {name}</div>
                <div class="index-ltp" style="color:{cfg['color']}">{ltp:,.2f}</div>
                <div class="{chg_cls}">{chg_sign}{chg_p:.2f}% &nbsp; ({chg_sign}{chg_abs:.2f})</div>
                <div style="margin-top:8px;font-size:11px;color:#607080">
                    H: <b style="color:#ddd">{high:,.2f}</b> &nbsp; L: <b style="color:#ddd">{low:,.2f}</b>
                </div>
                <div class="range-bar-outer" title="Day Range Position: {rp:.1f}%">
                    <div class="range-bar-inner" style="width:{rp_clipped}%;background:{bar_color}"></div>
                </div>
                <div style="font-size:10px;color:#445566">Range Pos: {rp:.1f}% &nbsp;|&nbsp; Gap: {gap_sign}{gap_p:.2f}%</div>
            </div>
            """, unsafe_allow_html=True)
    ...
    with v_col1:
        ...
        st.markdown(f"""
        <div class="vix-box">
            <div style="font-size:11px;color:#9b59b6;text-transform:uppercase;letter-spacing:1px">India VIX</div>
            <div class="vix-val">{vix_val:.2f}</div>
            <div style="color:{chg_col};font-size:13px;font-weight:700">{sign}{vix_chg:.2f}%</div>
            <div style="margin-top:6px"><span class="{badge_cls}">{vix_label}</span></div>
            <div style="font-size:10px;color:#6a3a9a;margin-top:4px">
                VIX &lt;15: Low Vol &nbsp;|&nbsp; &gt;20: Caution &nbsp;|&nbsp; &gt;25: Panic
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with v_col2:
        ...
        for tc, (name, cfg) in zip(t_cols, INDEX_CONFIG.items()):
            ...
            with tc:
                st.markdown(f"""
                <div class="param-block" style="text-align:center">
                    <div style="font-size:12px;color:#607080">{cfg['icon']} {name}</div>
                    <div style="margin:6px 0"><span class="{badge_class}">{tb['trend']}</span></div>
                    <div style="font-size:18px;font-weight:800;color:{score_color}">{tb['score']:+d}</div>
                    <div style="font-size:10px;color:#445566">Bias Score</div>
                </div>
                """, unsafe_allow_html=True)
    ...
    with tabs[4]:
        ...
        if ltp_sel:
            st.divider()
            st.markdown(f"### {cfg_sel['icon']} {sel_index} — Live Context")
            cx = st.columns(6)
            chg_delta = f"{s_sel.get('chg_pct',0):+.2f}%"
            cx[0].metric("LTP", f"₹{ltp_sel:,.2f}", chg_delta)
            cx[1].metric("Trend", tb_sel["trend"], f"Score {tb_sel['score']:+d}")
            pcr_v = o_sel.get("pcr", 0)
            cx[2].metric("PCR", f"{pcr_v:.3f}" if pcr_v else "—", pcr_signal(pcr_v)[0] if pcr_v else "")
            cx[3].metric("India VIX", f"{vix_val:.2f}", vix_signal(vix_val)[0])
            atr5_v = a_sel.get("atr5", 0)
            cx[4].metric("ATR 5D", f"{atr5_v:,.1f}" if atr5_v else "—",
                         f"Exp {a_sel.get('atr_expansion',0):.0f}%")
            cx[5].metric("Range Pos", f"{s_sel.get('range_pos',0):.1f}%",
                         "Bullish zone" if s_sel.get('range_pos',50) > 60 else "Bearish zone")
            st.divider()
        ...
        if ltp_sel and not chain_df.empty:
            ...
            if not trades:
                ...
                for i, t in enumerate(trades):
                    ...
                    st.markdown(f"""
<div style="border-left:5px solid {border_col};background:#111a27;border-radius:0 12px 12px 0;padding:16px 20px;margin:12px 0 4px 0">
<span style="font-size:24px;font-weight:900;color:{'#00e676' if t['direction']=='BUY' else '#ff5252'}">{dir_emoji}</span>
&nbsp;&nbsp;
<span style="font-size:22px;font-weight:900;color:{'#40c4ff' if t['option_type']=='CE' else '#ce93d8'}">{opt_emoji} {t['display_sym']}</span>
&nbsp;&nbsp;
<span style="background:{border_col}33;color:{border_col};border:1px solid {border_col};border-radius:6px;padding:3px 10px;font-size:13px;font-weight:700">{conf_emoji} CONFIDENCE</span>
&nbsp;&nbsp;
<span style="color:#556677;font-size:13px">{t['moneyness']} &nbsp;|&nbsp; Expiry: <b style="color:#889aaa">{t['expiry']}</b> &nbsp;|&nbsp; Lot: <b style="color:#889aaa">{t['lot_size']} units</b></span>
</div>""", unsafe_allow_html=True)

                    # ── 6 metric columns ────────────────────────────────────
                    m1, m2, m3, m4, m5, m6 = st.columns(6)
                    m1.metric("📥 Entry",    f"₹{t['entry']:.2f}",   "Current price")
                    m2.metric("🛑 Stop Loss", f"₹{t['sl']:.2f}",     f"−{t['sl_pct']:.0f}% from entry")
                    m3.metric("🎯 Target 1",  f"₹{t['target1']:.2f}", "Book 50% here")
                    m4.metric("🚀 Target 2",  f"₹{t['target2']:.2f}", "Trail SL after T1")
                    m5.metric("⚖️ R:R Ratio", f"1 : {t['rr_ratio']:.1f}", ">1:1.5 = good trade")
                    m6.metric("📊 OI",        f"{t['oi']:,}",          "Open Interest")
                    ...
                    p1.metric("💰 Premium / Lot",   f"₹{premium_per_lot:,}", "Capital needed")
                    p2.metric("🔴 Risk / Lot",       f"₹{risk_per_lot:,}",   "If SL hit")
                    p3.metric("🟢 Reward / Lot (T1)", f"₹{reward_per_lot:,}", "At Target 1")
                    p4.metric("📐 Bias Score",        f"{t['bias_score']:+d}", "−100 to +100")
