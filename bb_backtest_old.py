...
INSTRUMENT_MAP = {
    # ── MCX Futures ───────────────────────────────────────────────────────
    # exclude_prefix list now covers all mini/micro variants
...
# Trailing stop
TRAIL_STOP_PCT = 0.01          # 1.5%

# Minimum ATR filter — lower threshold = fewer skips
MIN_ATR_PCT   = 0.005           # was 0.005
...
def get_instrument_token(kite: KiteConnect, symbol: str):
    ...
    if inst_type == "FUT":
        ...
        if active.empty:
            raise RuntimeError(f"No {sym_upper} futures found on {exchange}")
